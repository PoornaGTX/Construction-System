//actions for alerts
export const DISPLAY_ALERT = "SHOW_ALERT";
export const CLEAR_ALERT = "CLEAR_ALERT";
//actions for register
export const REGISTER_UER_BEGIN = "REGISTER_UER_BEGIN";
export const REGISTER_UER_SUCCESS = "REGISTER_UER_SUCCESS";
export const REGISTER_UER_ERROR = "REGISTER_UER_END";
//actions for login
export const LOGIN_USER_BEGIN = "LOGIN_USER_BEGIN";
export const LOGIN_USER_SUCCESS = "LOGIN_USER_SUCCESS";
export const LOGIN_USER_ERROR = "LOGIN_USER_ERROR";
//action for toggle sidebar
export const TOGGLE_SIDEBAR = "TOGGLE_SIDEBAR";
//action for logout user
export const LOGOUT_USER = "LOGOUT_USER";
//Update user
export const UPDATE_USER_BEGIN = "UPDATE_USER_BEGIN";
export const UPDATE_USER_SUCCESS = "UPDATE_USER_SUCCESS";
export const UPDATE_USER_ERROR = "UPDATE_USER_ERROR";
//handleChange
export const HANDLE_CHANGE = "HANDLE_CHANGE";
//clear values
export const CLEAR_VALUES = "CLEAR_VALUES";
//create product
export const CREATE_PRODUCT_BEGIN = "CREATE_PRODUCT_BEGIN";
export const CREATE_PRODUCT_SUCCESS = "CREATE_PRODUCT_SUCCESS";
export const CREATE_PRODUCT_ERROR = "CREATE_PRODUCT_ERROR";
//get all jobs
export const GET_ALL_PRODUCTS_BEGIN = "GET_ALL_PRODUCTS_BEGIN";
export const GET_ALL_PRODUCTS_SUCCESS = "GET_ALL_PRODUCTS_SUCCESS";
//set edit job
export const SET_EDIT_PRODUCT = "SET_EDIT_PRODUCT";
//edit product
export const EDIT_PRODUCT_BEGIN = "EDIT_PRODUCT_BEGIN";
export const EDIT_PRODUCT_SUCCESS = "EDIT_PRODUCT_SUCCESS";
export const EDIT_PRODUCT_ERROR = "EDIT_PRODUCT_ERROR";
//delete jobs
export const DELETE_PRODUCT_BEGIN = "DELETE_PRODUCT_BEGIN";
//add to cart
export const ADD_TO_CART_BEGIN = "ADD_TO_CART";
export const ADD_TO_CART_SUCCESS = "ADD_TO_CART_SUCCESS";
export const ADD_TO_CART_ERROR = "ADD_TO_CART_ERROR";
//get all cart
export const GET_ALL_CART_BEGIN = "GET_ALL_CART_BEGIN";
export const GET_ALL_CART_SUCCESS = "GET_ALL_CART_SUCCESS";
//clear cart
export const CLEAR_CART = "CLEAR_CART";
//create product
export const CREATE_PROJECT_BEGIN = "CREATE_PROJECT_BEGIN";
export const CREATE_PROJECT_SUCCESS = "CREATE_PROJECT_SUCCESS";
export const CREATE_PROJECT_ERROR = "CREATE_PROJECT_ERROR";
//get all projects
export const GET_ALL_PROJECTS_BEGIN = "GET_ALL_PROJECTS_BEGIN";
export const GET_ALL_PROJECTS_SUCCESS = "GET_ALL_PROJECTS_SUCCESS";
//get all users
export const GET_ALL_USERS_BEGIN = "GET_ALL_USERS_BEGIN";
export const GET_ALL_USERS_SUCCESS = "GET_ALL_USERS_SUCCESS";
//delete projects
export const DELETE_PROJECT_BEGIN = "DELETE_PROJECT_BEGIN";

//set edit project
export const SET_EDIT_PROJECT = "SET_EDIT_PROJECT";
//edit project
export const EDIT_PROJECT_BEGIN = "EDIT_PROJECT_BEGIN";
export const EDIT_PROJECT_SUCCESS = "EDIT_PROJECT_SUCCESS";
export const EDIT_PROJECT_ERROR = "EDIT_PROJECT_ERROR";

export const GET_ALL_SUPP_ORDERS_SUCCESS = "GET_ALL_SUPP_ORDERS_SUCCESS";
// get all Orders Above One Lakh
export const GET_ALL_ORDERS_ABOVE_ONE_LAKH_BEGIN =
  "GET_ALL_ORDERS_ABOVE_ONE_LAKH_BEGIN";
export const GET_ALL_ORDERS_ABOVE_ONE_LAKH_SUCCESS =
  "GET_ALL_ORDERS_ABOVE_ONE_LAKH_SUCCESS";

export const SET_EDIT_APPROVE_ORDER = "SET_EDIT_APPROVE_ORDER";

export const EDIT_APPROVE_ORDER_BEGIN = "EDIT_APPROVE_ORDER_BEGIN";
export const EDIT_APPROVE_ORDER_SUCCESS = "EDIT_APPROVE_ORDER_SUCCESS";
export const EDIT_APPROVE_ORDER_ERROR = "EDIT_APPROVE_ORDER_ERROR";

export const SET_EDIT_DELIVER_ORDER = "SET_EDIT_DELIVER_ORDER";

